Scaling [iotsolution-grpc-534a-3f10-grpc-] With Kubernetes
===========================
...
Based on your TML solution [iotsolution-grpc-534a-3f10-grpc-] - if you want to scale your application with Kubernetes - you will need to apply the following YAML files.

.. list-table::

   * - **YML File**
     - **Description**
   * - :ref:`iotsolution-grpc-534a-3f10-grpc-.yml`
   ...
   * - :ref:`qdrant.yml`
     - This is the Qdrant deployment YAML.
       ...
       applied if using Step 9 DAG.
   * - :ref:`nginx-ingress-iotsolution-grpc-534a-3f10-grpc-.yml`
     - If you are scaling your TML solution you must

       apply the nginx-ingressiotsolution-grpc-534a-3f10-grpc-.yml; this yaml is 
...
.. code-block:: YAML
   ...
   kubectl apply -f kafka.yml -f secrets.yml -f mysql-storage.yml -f mysql-db-deployment.yml -f iotsolution-grpc-534a-3f10-grpc-.yml

iotsolution-grpc-534a-3f10-grpc-.yml
------------------------

.. important::
   Copy and Paste this YAML file: iotsolution-grpc-534a-3f10-grpc-.yml - and save it locally.
...
.. code-block:: YAML
   ...
   ################# iotsolution-grpc-534a-3f10-grpc-.yml
     ...
     kind: Deployment
     metadata:
       name: iotsolution-grpc-534a-3f10-grpc-
     spec:
       selector:
         matchLabels:
           app: iotsolution-grpc-534a-3f10-grpc-
       ...
       template:
         ...
         spec:
           containers:
           - name: iotsolution-grpc-534a-3f10-grpc-
             image: maadsdocker/iotsolution-grpc-534a-3f10-grpc--amd64:latest
             volumeMounts:
             - name: dockerpath
             ...
             - name: TSS
               value: '0'
             - name: PROJECTNAME
               value: 'iotsolution-grpc-534a-3f10'               
             - name: SOLUTIONNAME
               value: 'iotsolution-grpc-534a-3f10-grpc-'
             - name: SOLUTIONDAG
               value: 'solution_preprocessing_dag_grpc-iotsolution-grpc-534a-3f10'
             - name: GITUSERNAME
               value: 'smaurice101'
             - name: GITREPOURL
   ...
   ---
     ...
     kind: Service
     metadata:
       name: iotsolution-grpc-534a-3f10-grpc--visualization-service
       labels:
         app: iotsolution-grpc-534a-3f10-grpc--visualization-service
     spec:
       type: ClusterIP
       ports:
   ...
   ---
     ...
     kind: Service
     metadata:
       name: iotsolution-grpc-534a-3f10-grpc--external-service
       labels:
         app: iotsolution-grpc-534a-3f10-grpc--external-service
     spec:
       type: ClusterIP
       ports:
       - port: 443 # Ingress port, if using port 443 will need to setup TLS certs
         name: p2
         protocol: TCP
         targetPort: 9002
       selector:
...
.. code-block::
   ...
   kubectl port-forward deployment/iotsolution-grpc-534a-3f10-grpc- 6060:6060

After you forward the ports then copy/paste the viusalization URL below and run your dashboard.

.. code-block::

   http://localhost:6060/iot-failure-seneca.html?topic=iot-preprocess,iot-preprocess2&offset=-1&groupid=&rollbackoffset=400&topictype=prediction&append=0&secure=1
...
.. important::
   ...
   **STEP 4:  Apply nginx-ingress-iotsolution-grpc-534a-3f10-grpc-.yml to your kubernetes cluster.  First you need to save it locally then apply it:**

nginx-ingress-iotsolution-grpc-534a-3f10-grpc-.yml
-------------

   .. code-block::

      
    ############# nginx-ingress-iotsolution-grpc-534a-3f10-grpc--grpc.yml
   ...
   .. code-block::
      ...
      kubectl apply -f nginx-ingress-iotsolution-grpc-534a-3f10-grpc-.yml
...
.. code-block::
   ...
   http://tml.tss2/viz/iot-failure-seneca.html?topic=iot-preprocess,iot-preprocess2&offset=-1&groupid=&rollbackoffset=400&topictype=prediction&append=0&secure=1
